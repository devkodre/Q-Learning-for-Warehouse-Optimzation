...
with open("/Users/devkodre/My project/Assets/cubes.txt", "w") as file:
    ...
    while len(spaces) != 0:
        ...
        if state in qTable:
            bestPos = max(qTable[state], key=lambda key: qTable[state][key])
            policy = [bestPos[0], bestPos[1], bestPos[2]]
            updatePositions((policy,currentSize)) # updates spaces
            cubes.append((policy,currentSize))
        else:
            print("Not in Table")
            ...
            for pos in legals:
                if (pos[0],pos[1],pos[2]) not in qTable[state]:
                    qTable[state][(pos[0],pos[1],pos[2])] = 0
        ...
        print(f"Policy: {policy} Size: {currentSize}")
